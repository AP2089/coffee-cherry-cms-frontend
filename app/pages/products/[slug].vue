        ...
        <Card class="border-border">
          <CardContent class="space-y-5 p-4 md:p-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
              <div class="space-y-2">
                <Label>Slug</Label>
                <Input :model-value="slug" disabled />
              </div>
              <div class="space-y-2">
                <Label>Цена (₽ за 250 г)</Label>
                <Input v-model.number="commerce.price" type="number" min="0" />
              </div>
              <div class="space-y-2">
                <Label>Остаток</Label>
                <Input v-model.number="commerce.stock" type="number" min="0" />
              </div>
              <div class="space-y-2">
                ...
                <div class="flex flex-wrap gap-2">
                  <Button
                    ...
                    type="button"
                    size="sm"
                    :variant="commerce.weights.includes(weight) ? 'magnetic-filled' : 'outline'"
...
import type { CoffeeCommerceFields, CoffeeTextFields, CoffeeWeight } from '~/types/product'
...
const commerce = ref<CoffeeCommerceFields>({
  price: 0,
  stock: 0,
...
function syncCommerceFields() {
  ...
  commerce.value = {
    ...
    stock: products.current.stock,
    weights: [...products.current.weights],
  }
}
...
function toggleWeight(weight: CoffeeWeight) {
  if (commerce.value.weights.includes(weight)) {
    if (commerce.value.weights.length === 1) return
    commerce.value.weights = commerce.value.weights.filter((item) => item !== weight)
    return
  }

  commerce.value.weights = [...commerce.value.weights, weight].sort((a, b) => a - b)
}
...
async function saveProduct() {
  ...
  try {
    await products.saveProduct(slug.value, activeLocale.value, form.value, commerce.value)
